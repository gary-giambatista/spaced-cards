...
function Deck_Adder({ drawerOpen, setSelectedDeck, decks, setDecks }) {
	...
	return (
		<div
			className={`flex items-center py-2 gap-2 overflow-hidden px-4 ${
		...
		>
			...
			{isAddingDeck ? (
					...
					{newDeckName ? (
						<button onClick={() => createDeck()}>
							<svg
								...
								viewBox="0 0 24 24"
								strokeWidth={1.5}
								className="w-7 h-7 bg-white rounded-md stroke-green-500 border border-green-500"
							>
								<path
									strokeLinecap="round"
						...
						<button onClick={() => setIsAddingDeck((prevState) => !prevState)}>
							<svg
								...
								viewBox="0 0 24 24"
								strokeWidth={1.5}
								className="w-7 h-7 bg-white rounded-md stroke-red-500 border border-red-500  dark:stroke-black "
							>
								<path
									strokeLinecap="round"
									strokeLinejoin="round"
				...
				<>
					<div className="line-clamp-1 leading-10 mr-auto text-lg">
						Create Deck
					</div>
					<button
						onClick={() => {
					...
					>
						<svg
							...
							viewBox="0 0 20 20"
							fill="currentColor"
							className="w-7 h-7 bg-neutral-100 dark:bg-transparent rounded-md fill-black dark:fill-white"
						>
